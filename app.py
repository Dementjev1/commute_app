...
def get_ai_recommendation(data):
    ...
    # TRY THIS SPECIFIC ID: gemini-1.5-flash-latest
    model_id =model_id = 'gemini-2.0-flash-lite'

    # 1. First, let's try WITHOUT the JSON mime_type to see if it clears the 404
    # If this works, the problem was the JSON constraint versioning.
    ...
    try:
        for event in data['calendar_events']:
            ...
            # Since we commented out mime_type, we use .text and parse manually
...
# --- TEST IT ---
# print(get_ai_recommendation(your_data_object))